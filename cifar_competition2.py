...
def main(args: argparse.Namespace) -> None:
    ...
    # Load data
    cifar = CIFAR10()
    d = cifar.train.dataset(lambda x: (augmentation_fn(x['images']), x['labels']))
    ...
    for i in range(0,0):
        image = cifar.train.data['images'][120]
        label = cifar.train.data['labels'][120]
        
        transformed_image = d[120][0]
    ...
    # Train the model
    train = torch.utils.data.DataLoader(d, batch_size=args.batch_size, shuffle=True)
    dev =  torch.utils.data.DataLoader(cifar.dev, batch_size=args.batch_size)
    for i in range(0,1):
